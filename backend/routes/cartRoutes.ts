...
router.post('/save', async (req, res) => {
  ...
  try {
    await Cart.upsert({ userId, items: JSON.stringify(items) }); // Convert items array to JSON string before saving
...
router.get('/:userId', async (req, res) => {
  try {
    ...
    const cartItems = JSON.parse(cart.items); // Parse JSON string back into array
    res.json({ cartItems });
  } catch (error) {
    console.error(error);
...
router.post('/add-item', async (req, res) => {
  ...
  try {
    ...
    const cartItems = JSON.parse(cart.items); // Parse existing items
